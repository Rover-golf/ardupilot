...
uint8_t Make_Packet(uint8_t *CmdBuff, Packet *pk, Payload *pl)
{
	...
	uint16_t PayloadSize1 = ((CmdBuff[POS_LENGTH_2] << 8) & 0xff00) | (CmdBuff[POS_LENGTH_1] & 0x00ff);
	uint8_t CheckSum1 = CmdBuff[PayloadSize1 + HEADER_LENGTH_SIZE];
	Payload_Parsing(&CmdBuff[POS_PAYLOAD_HEADER], PayloadSize1, pl);

	pk->Header1 = FirstBit;
	pk->Header2 = SecondBit;
	pk->Header3 = ThirdBit;
	pk->Length = PayloadSize1;
	pk->Payload = &CmdBuff[POS_PAYLOAD_HEADER];
	pk->Checksum = CheckSum1;
