...
void AP_RangeFinder_Benewake_TFMiniPlus::update()
{
    ...
    if (accum.count > 0) {
        ...
        accum.count = 0;
        update_status();
     //   gcs().send_text(MAV_SEVERITY_INFO, "distance =  %d", state.distance_cm);
    } else if (AP_HAL::millis() - state.last_reading_ms > 200) {
        set_status(RangeFinder::Status::NoData);
      //  gcs().send_text(MAV_SEVERITY_INFO, "distance =  NoData");
    }
}
...
{
    //GetLidarData();
    //gcs().send_text(MAV_SEVERITY_INFO, "GetLidarData");
    //return;
...
//LUNA READ
void AP_RangeFinder_Benewake_TFMiniPlus::GetLidarData() {
    uint8_t REG_DIST_L_00H = 0x00;
    ...
    ret = _dev->transfer(&REG_DIST_L_00H, 1, nullptr, 0);
    if (!ret || !_dev->transfer(nullptr, 0, rx_buf, sizeof(rx_buf))) {
        return;
    }	
	lidar.u16Distance = rx_buf[1] * 256 + rx_buf[0];
	lidar.u16Amp = rx_buf[3] * 256 + rx_buf[2];
	lidar.u16Temp = rx_buf[5] * 256 + rx_buf[4];
    if(lidar.u16Distance < 1200)
    {
        accum.sum += lidar.u16Distance;
        accum.count++;        
    }
}
