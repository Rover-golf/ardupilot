...
{
    // debug info
    int imode = (int)control_mode->mode_number();
    
    if(old_imode != imode &&(imode == 10 || imode == 11))//auto or rtl mode enable fence
    {
        if(!fence.enabled())
        {
            fence.enable(true,AC_FENCE_ALL_FENCES);
    ...
    if (g.golf_timing_enable == 1)
    {
        ...
        if(!isSleep && pi_ctl)
            gosleeping = time_Check(hour, min, (uint8_t)g.end_1_hour,(uint8_t)g.end_1_min,5);  
        ...
        if(!isSleep && pi_ctl)
            gosleeping |= time_Check(hour, min, (uint8_t)g.end_2_hour,(uint8_t)g.end_2_min,5);
        ...
        if(!isSleep && pi_ctl)
            gosleeping |= time_Check(hour, min, (uint8_t)g.end_3_hour,(uint8_t)g.end_3_min,5);
...
void Rover::golf_send_cmd(uint16_t cmd_id, const float &param1, const float &param2)
{
    ...
    for (uint8_t i = 0; i < SERIALMANAGER_NUM_PORTS; i++) 
    {
        ...
        if (uart != nullptr ) 
        {
            switch (serial_manager.get_state_by_id(i)->protocol) 
            {
