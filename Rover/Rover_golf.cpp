...
void Rover::hundred_hz_loop(void)
{
    ...
    if (yaw_enable)
    {
        ...
        */
        ...
        float throttle = 0;
        if(fabsf(yaw_rate_to) > g.steer_rate_use * 0.9 && pie_ctl_times > 10) //4050 when rover can't rotate even though the steer is up to the max
            throttle = g.golf_forward + pie_ctl_times;   
        
        rover.mode_gobatt.set_para(throttle, yaw_rate_to);
        gcs().send_text(MAV_SEVERITY_INFO, "gps yaw_desire=%.0f yaw_get=%.0f throttle=%.0f turn=%.0f", yaw_desire, yaw_get,throttle, yaw_rate_to);
        ...
        case 9000: // reached home then unload ball
        {
            ...
            case 8:
                // backward
                {
                    ...
                    g2.beacon.get_data(dis, angle);
                    dis = dis * 100; // m->cm
                    if (dis > 500 || one_hz_times > g.golf_time_backward) // 12
                    {
                        rover.mode_gobatt.set_para(); // stop
                        pi_ctl_step = 0;
                        pi_ctl = false;
                    ...
                    else
                    {
                        ...
                        float f = g.golf_throttleR + pie_ctl_times;
                        rover.mode_gobatt.set_para(-f);
                        gcs().send_text(MAV_SEVERITY_INFO, "times= %d, dis= %.f, backward= %.0f", pie_ctl_times, dis, -f);
                    }                                      
                }
