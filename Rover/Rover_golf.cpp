...
//    gcs().send_text(MAV_SEVERITY_INFO, "Mode= %d, work_enable= %d, isSleep=%d",imode, work_enable,isSleep);
 //   gcs().send_text(MAV_SEVERITY_INFO, "golf_work_state = %d ", golf_work_state);
    ...
    //uwbmsg
 //   Vector3f pos;
 //   g2.beacon.get_position(pos);
 //   gcs().send_text(MAV_SEVERITY_INFO, "UWB rover x= %f, y=%f, z=%f", pos.x,pos.y,pos.z);
...
//#endif
    ...
    {
        bool time1_avaiable = !((g.start_1_hour == 0) && (g.start_1_min == 0) && (g.end_1_hour == 0) && (g.end_1_min == 0));
        bool time1_to_start = ((hour == (uint8_t)g.start_1_hour) && (min == (uint8_t)g.start_1_min) && (sec < 4));
        //    bool time1_to_start = ((hour*24+min >= (uint8_t)g.start_1_hour*24+(uint8_t)g.start_1_min)
        //                        && (hour*24+min < (uint8_t)g.end_1_hour*24 +(uint8_t)g.end_1_min) && (sec < 4));
        bool time1_to_end = ((hour == (uint8_t)g.end_1_hour) && (min == (uint8_t)g.end_1_min) && (sec < 4));
        bool time2_avaiable = !((g.start_2_hour == 0) && (g.start_2_min == 0) && (g.end_2_hour == 0) && (g.end_2_min == 0));
        bool time2_to_start = ((hour == (uint8_t)g.start_2_hour) && (min == (uint8_t)g.start_2_min) && (sec < 4));
        //    bool time2_to_start = ((hour*24+min >= (uint8_t)g.start_2_hour*24+(uint8_t)g.start_2_min)
        //                        && (hour*24+min < (uint8_t)g.end_2_hour*24 +(uint8_t)g.end_2_min) && (sec < 4));
        bool time2_to_end = ((hour == (uint8_t)g.end_2_hour) && (min == (uint8_t)g.end_2_min) && (sec < 4));
        bool time3_avaiable = !((g.start_3_hour == 0) && (g.start_3_min == 0) && (g.end_3_hour == 0) && (g.end_3_min == 0));
        bool time3_to_start = ((hour == (uint8_t)g.start_3_hour) && (min == (uint8_t)g.start_3_min) && (sec < 4));
        //    bool time3_to_start = ((hour*24+min >= (uint8_t)g.start_3_hour*24+(uint8_t)g.start_3_min)
        //                        && (hour*24+min < (uint8_t)g.end_3_hour*24 +(uint8_t)g.end_3_min) && (sec < 4));
        bool time3_to_end = ((hour == (uint8_t)g.end_3_hour) && (min == (uint8_t)g.end_3_min) && (sec < 4));

        gcs().send_text(MAV_SEVERITY_INFO, "s1h=%d s1m=%d, e1h=%d,e1m=%d", (int)g.start_1_hour, (int)g.start_1_min, (int)g.end_1_hour,(int)g.end_1_min);
        gcs().send_text(MAV_SEVERITY_INFO, "t1s=%i t1e=%i, t1a=%i", time1_to_start, time1_to_end, time1_avaiable);
